...
| Breakpoints pass | ✅ no change needed | Sweep of `css/base.css` + `css/main.css` + all templates: no hard-coded Tailwind-default-matching breakpoints remain (the legacy `hide-md` 53.125rem→48rem→`max-md:` move happened in the header pass). Only media queries left are the two documented exceptions — `@media (max-width: 32rem)` (aside sprite) and `@media (max-width: 60rem)` (`.flex` restack) — both non-standard values, single-use, deliberately kept as plain `@media` (`max-[]:` variants compile to `width <` negation, boundary differs at exactly 512px/960px). No `--breakpoint-*` tokens added (nothing non-standard is reused) |
...
| Colours → OKLCH primitives | ♻️ migrated | All `@theme` colour values standardised to OKLCH. New primitives `--color-white`/`--color-black` (the only raw colour values) hold the repeated `#fff`/`#000`; every other token derives from a primitive or is a unique brand colour: solid tokens via `var()`, alpha tokens via `color-mix(in oklab, … , transparent)` (Tailwind v4 compiles these to the original hex fallback + `@supports` progressive enhancement — resolved colours byte-identical, no visual change). Brand colours converted with verified 2-decimal OKLCH values that round-trip to the exact sRGB originals. Both brand gradients (`.primary` rule, interests `img` utility) pinned with `linear-gradient(in srgb 180deg, …)` — the interpolation method and angle share the first comma-free segment per the `linear-gradient()` grammar (`in srgb, 180deg, …` is invalid: the stray comma pushes the angle into the color-stop list and the whole declaration is dropped, leaving the solid/`none` fallback). Without the pin, OKLCH endpoints would flip CSS Color 4 smart-default interpolation to OKLab and shift the orange→purple mid-tones; the standard-syntax declaration before it stays as a fallback for engines without `<color-interpolation-method>` support (mid-tones interpolate in OKLab there — subtle shift, never transparency) |
